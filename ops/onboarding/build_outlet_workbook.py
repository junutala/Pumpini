...
order = [
    ...
    ("2. Outlet",        "The station: name, oil company, address and GSTIN as they should print on your invoices."),
    ("3. Staff (Mgr/CCO)","Managers and back-office (CCO) logins linked to this outlet."),
    ("4. Attendants",    "The operators who run the pumps and settle shifts. Name + mobile is enough."),
    ("5. Tanks",         "Underground tanks: number, fuel, capacity, opening stock, density + opening gauge/ATG photo."),
    ("6. Pumps",         "The dispenser machines + their SAMPLE TOTALIZER-SLIP PHOTO (required for slip scanning)."),
    ("7. Nozzles",       "Each nozzle mapped to its tank + fuel (and optionally its dispenser / slip line)."),
    ("8. Fuel Prices",   "Opening per-litre price for each fuel this outlet sells."),
    ("9. Shift Timings", "The 1–3 daily shift windows the outlet runs."),
    ("10. Credit Customers","Corporate / credit accounts and their go-live opening balances."),
    ("11. Credit Slip Books","Coupon booklets allocated to a credit customer: series range + a sample coupon photo."),
    ("12. Products",     "OPTIONAL — shop / lubricant products, only if the outlet uses the store module."),
    ("13. Defaults",     "OPTIONAL — LEAVE BLANK. Alert and tolerance defaults; we set these and you can change them in Settings."),
]
r = 6
...
img_lines = [
    ...
    "Pump sample slip (green column on tab 6) — REQUIRED for slip scanning. A photo of the totalizer slip the "
    ...
    "Tank gauge / ATG (Pinelabs) screen (green column on tab 5) — recommended at go-live. A photo of the gauge screen "
    "showing opening stock; it's the picture behind the first wet-stock figure the outlet is measured against.",
    "Credit-coupon sample (green column on tab 11) — one photo per credit customer's booklet. It records the SLIP "
...
specs = [
    ...
    ("Address\n(stations.address)", 30, "opt", "Street address. PRINTED ON YOUR GST INVOICES."),
    ("City\n(stations.city)", 14, "opt", "Printed on your GST invoices."),
    ("State\n(stations.state)", 16, "opt", "Printed on your GST invoices (place of supply)."),
    ("Pincode\n(station_settings.pincode)", 10, "opt", "Printed on your GST invoices."),
    ("GSTIN\n(gst_number / gstn)", 18, "opt", "15-char GST number. Printed on your GST invoices. Your PAN is already inside it (characters 3-12), so we do not ask for PAN separately."),
    ("Owner WhatsApp\n(owner_whatsapp)", 16, "opt", "10 digits. Where owner alerts and daily reports go."),
    ("Language\n(language)", 11, "opt", "Manager screen language. en / te / ta / hi. Default en."),
    ("Invoice prefix\n(invoice_prefix)", 13, "opt", "Prefix on your GST invoice numbers. Default INV."),
]
header_row(ws, 4, specs)
example_row(ws, 5, ["Kamala Highway", "HPCL", "NH-44, Kompally", "Hyderabad", "Telangana",
                    "500014", "36ABCDE1234F1Z5", "9876500001", "en", "KAM"])
...
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("3. Staff (Mgr-CCO)")
ws.sheet_view.showGridLines = False
sheet_title(ws, "3 · Managers & Back-office (CCO)",
...
# 5. ATTENDANTS
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("4. Attendants")
ws.sheet_view.showGridLines = False
sheet_title(ws, "4 · Attendants (operators)",
...
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("5. Tanks")
ws.sheet_view.showGridLines = False
sheet_title(ws, "5 · Tanks",
            "One row per underground tank. 'Opening stock' is the dip-verified stock at go-live. Tank number is "
            "referenced by nozzles on tab 7. The green column is the file name / link of the opening gauge photo.", 6)
specs = [
...
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("6. Pumps")
ws.sheet_view.showGridLines = False
sheet_title(ws, "6 · Pumps / Dispensers  (+ sample slip photo)",
...
# 8. NOZZLES
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("7. Nozzles")
ws.sheet_view.showGridLines = False
sheet_title(ws, "7 · Nozzles",
...
specs = [
    ...
    ("Pump no\n(→ pumps.pump_number)", 14, "opt", "OPTIONAL. Which dispenser (tab 6) this nozzle sits on."),
...
# 9. FUEL PRICES
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("8. Fuel Prices")
ws.sheet_view.showGridLines = False
sheet_title(ws, "8 · Opening Fuel Prices",
...
# 10. SHIFT TIMINGS
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("9. Shift Timings")
ws.sheet_view.showGridLines = False
sheet_title(ws, "9 · Shift Timings",
...
# 11. CREDIT CUSTOMERS
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("10. Credit Customers")
ws.sheet_view.showGridLines = False
sheet_title(ws, "10 · Credit (Corporate) Customers",
...
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("11. Credit Slip Books")
ws.sheet_view.showGridLines = False
sheet_title(ws, "11 · Credit Slip Books (coupon booklets)",
            ...
            "photo that records the slip format. Company name must match a row on tab 10.", 8)
specs = [
    ("Company name\n(→ Credit Customers)", 26, "req", "Which credit customer this booklet is issued to. Must match a company on tab 10."),
...
# ══════════════════════════════════════════════════════════════════════════
# 13. DEFAULTS  (OPTIONAL — leave blank)
# ══════════════════════════════════════════════════════════════════════════
ws = wb.create_sheet("13. Defaults")
ws.sheet_view.showGridLines = False
sheet_title(ws, "13 · Defaults  (OPTIONAL — LEAVE THIS TAB BLANK)",
            "You do not need to fill anything in here. These are Pumpini's alert and tolerance settings; "
            "we apply the defaults shown in the example row at go-live and you can change any of them yourself "
            "later in Settings. Fill a cell only if you already know you want a different value.", 11)
specs = [
    ("Variance threshold ₹\n(variance_threshold)", 15, "opt", "Cash short/over alert threshold. Default 50."),
    ("Latitude\n(latitude)", 12, "opt", "Only needed if you switch geofenced attendance on."),
    ("Longitude\n(longitude)", 12, "opt", "Only needed if you switch geofenced attendance on."),
    ...
    ("Tally company\n(tally_company_name)", 20, "opt", "Only if you export to Tally. Company name as it appears in Tally."),
]
header_row(ws, 4, specs)
example_row(ws, 5, [50, "17.5810", "78.4990", 500, "no", "no", 0.75, 0.50, 20, 2, "Kamala Highway HPCL"])
input_rows(ws, 6, specs, n=2)
for colL in ("E", "F"):
    add_dv(ws, colL, ["yes", "no"], 6)
ws.freeze_panes = "A5"

out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "docs", "New-Outlet-Setup.xlsx")
