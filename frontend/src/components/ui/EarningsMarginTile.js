...
export default function EarningsMarginTile({ stationId, maxRows = 10 }) {
  ...
  const hasMargin = margin.totals.margin !== 0 || margin.fuels.some(f => f.margin != null);
  ...
  return (
    <div className="card" style={{ marginBottom: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
        <div style={{ fontWeight: 700, fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 400, color: 'var(--text-3)' }}>
            · {tc('margin_tile.subtitle', 'gross margin vs weighted-avg purchase cost from Deliveries')}
          </span>
        </div>
      ...
      <div className="stack-mobile" style={{ display: 'grid', gridTemplateColumns: `minmax(160px,auto) repeat(${Math.max(margin.fuels.length, 1)}, 1fr)`, gap: 10, marginBottom: '1rem' }}>
      ...
      {events.map((ev, i) => (
        <div key={ev.id} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12, color: 'var(--text-3)', whiteSpace: 'nowrap' }}>{toIST(ev.occurred_at)}</span>
            <span className={`fuel-chip fuel-${ev.fuel_type}`}>
