...
export default function ManagerReconcileModal({ shift, onClose, onClosed }) {
  ...
  const save = async (a) => {
    ...
    if (f.closing_reading === '' || f.closing_reading == null) return alert('Enter the closing meter reading');
    if (!price) return alert('Enter the price per litre');
    setBusy(aid);
    try {
      const denomination = {};
    ...
    } catch (e) { alert(e.response?.data?.error || e.error || 'Failed to reconcile'); }
    setBusy('');
  };
  ...
  const closeShift = async () => {
    ...
    try {
      ...
      onClosed && onClosed();
    } catch (e) { alert(e.error || 'Failed to close shift'); }
    setClosing(false);
  };
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem', width: '100%', maxWidth: 560,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
          <h2 style={{ fontWeight: 800, fontSize: 17, margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Lock size={16} color="#FF6B00" /> Manager Reconciliation
          </h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer' }}><X size={18} /></button>
        </div>
        <p style={{ fontSize: 12.5, color: '#666', margin: '0 0 1rem' }}>
          Count cash with the operator and enter readings. Expected cash is derived from the meter — the operator never saw it.
        </p>

        {attendants.length === 0 && <div style={{ color: '#999', fontSize: 13, padding: '1rem 0' }}>No attendants assigned to this shift.</div>}
            ...
            <div key={aid} style={{ border: '1px solid #e5e3de', borderRadius: 12, padding: '0.9rem', marginBottom: '0.85rem',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                {r && <span style={{ fontSize: 12, fontWeight: 700, color: short ? '#dc2626' : '#16a34a', display: 'flex', alignItems: 'center', gap: 4 }}>
                  {short ? <AlertTriangle size={13} /> : <CheckCircle size={13} />}{short ? 'Shortage' : over ? 'Overage' : 'Tallied'}
                </span>}
              </div>
              <div style={{ fontSize: 11.5, color: '#888', marginBottom: 8 }}>
                Opening meter: <strong>{Number(a.opening_reading || 0).toFixed(3)}</strong> · Opening cash: <strong>{fmt(a.opening_cash)}</strong>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, marginBottom: 8 }}>
                <div><label style={{ fontSize: 11, fontWeight: 600 }}>Closing meter *</label>
                  <input style={inp} type="number" step="0.001" value={f.closing_reading || ''} onChange={e => setF(aid, 'closing_reading', e.target.value)} /></div>
                <div><label style={{ fontSize: 11, fontWeight: 600 }}>Price / litre *</label>
              ...
              <div style={{ fontSize: 12.5, marginBottom: 8 }}>Cash counted: <strong>{fmt(counted)}</strong></div>

              {/* Result */}
              {r && (
                <div style={{ background: '#fff', border: '1px solid #e5e3de', borderRadius: 8, padding: '8px 10px', marginBottom: 8, fontSize: 12.5 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Expected cash</span><strong>{fmt(r.cash_expected)}</strong></div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Counted cash</span><strong>{fmt(r.cash_actual)}</strong></div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', color: short ? '#dc2626' : '#16a34a', fontWeight: 700, marginTop: 2 }}>
                    <span>{short ? 'Shortage' : over ? 'Overage (booked as Other Income)' : 'Variance'}</span><span>{fmt(Math.abs(variance))}</span>
                  </div>
                </div>
              )}
              ...
              {short && (
                <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '8px 10px', marginBottom: 8 }}>
                  <div style={{ fontSize: 11.5, fontWeight: 600, color: '#9a3412', marginBottom: 6 }}>How is the shortage settled?</div>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                    {[['recovered', 'Cash recovered'], ['salary_deduction', 'Deduct from salary']].map(([id, label]) => (
              ...
              <button onClick={() => save(a)} disabled={busy === aid}
                ...
                {busy === aid ? 'Saving…' : r ? 'Recompute' : 'Reconcile'}
              </button>
            </div>
          );
        })}
        ...
        <button onClick={closeShift} disabled={!allDone || closing}
          ...
          {closing ? 'Closing…' : allDone ? 'Close Shift' : `Reconcile all attendants first (${Object.keys(results).length}/${attendants.length})`}
        </button>
      </div>
    </div>
  );
