    ...
    <AppShell>
      <div className="page-header">
        <div>
          ...
          {unreadCount > 0 && <div style={{ fontSize: 13, color: 'var(--danger)', marginTop: 2 }}>{unreadCount} unacknowledged</div>}
        </div>
      ...
      {loading && <div style={{ textAlign: 'center', color: 'var(--text-3)', padding: '3rem' }}>{t('loading')}</div>}

      {!loading && alerts.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          <Bell size={32} color="var(--text-3)" style={{ margin: '0 auto 12px' }} />
          <div style={{ color: 'var(--text-3)' }}>No alerts {unreadOnly ? 'unread' : 'recorded'}</div>
        </div>
      )}
            ...
            <div key={alert.id} className={`alert-banner ${s.banner}`}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                  ...
                  {isRead && <span className="badge badge-gray" style={{ fontSize: 11 }}>acknowledged</span>}
                </div>
