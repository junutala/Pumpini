...
test('a fully commissioned nozzle wants nothing', () => {
  assert.deepStrictEqual(
    wantsFor({ pump_id: 'p1', pump_serial: '17EH2900V', slip_nozzle_no: '1', events: 2 }), []);
});

test('no serial on file is named', () => {
  assert.ok(wantsFor({ pump_id: 'p1', pump_serial: null, slip_nozzle_no: '1', events: 1 }).includes('serial'));
});
...
test('a missing printed number is named, not defaulted', () => {
  assert.deepStrictEqual(
    wantsFor({ pump_id: 'p1', pump_serial: '17EH2900V', slip_nozzle_no: '', events: 1 }), ['printed_no']);
  assert.deepStrictEqual(
    wantsFor({ pump_id: 'p1', pump_serial: '17EH2900V', slip_nozzle_no: '   ', events: 1 }), ['printed_no']);
});
...
test('no genesis event means not commissioned, however complete the pair looks', () => {
  assert.deepStrictEqual(
    wantsFor({ pump_id: 'p1', pump_serial: '17EH2900V', slip_nozzle_no: '1', events: 0 }), ['genesis']);
});
...
test('a printed number of "0" counts as read', () => {
  assert.deepStrictEqual(
    wantsFor({ pump_id: 'p1', pump_serial: 'M1832105', slip_nozzle_no: '0', events: 1 }), []);
});

test('a bare nozzle wants the pump first, then the rest', () => {
  assert.deepStrictEqual(wantsFor({}), ['pump', 'printed_no', 'genesis']);
});

// A NOZZLE BELONGING TO NO PUMP has nowhere for a serial to go, and reporting "no
// serial" would send the owner to type one into a screen that cannot store it.
test('a nozzle with no pump asks for the pump, not for a serial', () => {
  const w = wantsFor({ pump_id: null, slip_nozzle_no: '1', events: 1 });
  assert.deepStrictEqual(w, ['pump']);
  assert.ok(!w.includes('serial'));
});

test('a nozzle with a pump but no serial still asks for the serial', () => {
  assert.deepStrictEqual(
    wantsFor({ pump_id: 'p1', pump_serial: '', slip_nozzle_no: '1', events: 1 }), ['serial']);
});
